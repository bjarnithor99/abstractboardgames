...
if __name__ == "__main__":
    ...
    b_w_wins, b_w_draws, b_w_loss = tournament([agent, RandomAgent()], 50)
    b_b_loss, b_b_draws, b_b_wins = tournament([RandomAgent(), agent], 50)
    ...
    a_w_wins, a_w_draws, a_w_loss = tournament([agent, RandomAgent()], 50)
    a_b_loss, a_b_draws, a_b_wins = tournament([RandomAgent(), agent], 50)

    log.info("Results")
    log.info(f"Pre-training as white (wins, draw, loss): {b_w_wins} {b_w_draws} {b_w_loss}")
    log.info(f"Pre-training as black (wins, draw, loss): {b_b_wins} {b_b_draws} {b_b_loss}")
    log.info(f"Post-training as white (wins, draw, loss): {a_w_wins} {a_w_draws} {a_w_loss}")
    log.info(f"Post-training as black (wins, draw, loss): {a_b_wins} {a_b_draws} {a_b_loss}")
